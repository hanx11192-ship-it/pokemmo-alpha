...
│   │   ├── lanbizi.py      # <span lang="zh">纪念源适配器（已停用，作模板保留）</span>
│   │                       # <span lang="en">Tribute adapter (disabled, kept as template)</span>
│   │   ├── lzpoke_reports.py     # LZPoke <span lang="zh">报点源</span> <span lang="en">LZPoke report source</span>
│   │   └── pokemmotools_landing.py  # PokemmoTools <span lang="zh">活跃头目源</span> <span lang="en">PokemmoTools landing source</span>
...
# <span lang="zh">海外源站通过 config/sources.yaml 的 options.proxy 配置隧道代理，此处无需设置</span>
# <span lang="en">Overseas sources use options.proxy in config/sources.yaml; no env needed here</span>
python3 -m src.main
```
...
| <span lang="zh">（源级代理）</span> <span lang="en">(per-source proxy)</span> | <span lang="zh">在 config/sources.yaml 的 options.proxy 配置，如 http://127.0.0.1:8899</span> <span lang="en">Set options.proxy in config/sources.yaml</span> | <span lang="zh">海外源站必填</span> <span lang="en">Required for overseas sources</span> |
| `NO_PROXY` | <span lang="zh">不走代理的地址（默认 127.0.0.1,localhost）</span> <span lang="en">Bypass-proxy hosts</span> | <span lang="zh">否</span> <span lang="en">No</span> |
...
<span lang="zh">> 注：旧版 `transfor_url` / `PROXY_KEY`（PHP 中转）已废弃——虚拟主机未备案，出站被 IDC 拦截。海外源站现统一改用 `config/sources.yaml` 里的 `options.proxy` 指向隧道代理（如阿里云 autossh 隧道 `127.0.0.1:8899`）。</span>
<span lang="en">> Note: the old `transfor_url` / `PROXY_KEY` (PHP relay) is deprecated — the unregistered virtual host was blocked by the IDC. Overseas sources now use `options.proxy` in `config/sources.yaml` pointing to a tunnel proxy (e.g. the Aliyun autossh tunnel at `127.0.0.1:8899`).</span>
...
<span lang="zh">**完整开发文档与模板：**</span> <span lang="en">**Full dev guide & template:**</span>

<span lang="zh">详见 **[`适配器开发指南.md`](适配器开发指南.md)**，以及示例适配器模板 **[`mycrawler.py`](mycrawler.py)**（把你的爬虫转成适配器的完整范例）。`src/sources/lanbizi.py` 仍保留作参考模板。</span>
<span lang="en">See **[`适配器开发指南.md`](适配器开发指南.md)** for the full guide, and the example adapter **[`mycrawler.py`](mycrawler.py)** (a complete example turning your crawler into an adapter). `src/sources/lanbizi.py` is kept as a reference template.</span>
...
## <span lang="zh">适配器开发</span> <span lang="en">Adapter Development</span>

<span lang="zh">
想接入自己的数据源，或把已有的爬虫接进面板？详见仓库内的 **[`适配器开发指南.md`](适配器开发指南.md)**——从 `BossData` 契约、`FetchResult` 三态、到七步添加新源、爬虫转适配器的完整实战，都讲清楚了。配套示例 **[`mycrawler.py`](mycrawler.py)** 是一个「把爬虫输出翻成 `BossData`」的可运行模板，照着改 `_crawl()` 就能上线。
</span>

<span lang="en">
Want to plug in your own data source, or turn an existing crawler into an adapter? See **[`适配器开发指南.md`](适配器开发指南.md)** in the repo — it covers the `BossData` contract, the `FetchResult` three-state result, a seven-step guide to adding a new source, and a full crawler-to-adapter walkthrough. The companion **[`mycrawler.py`](mycrawler.py)** is a runnable template that turns crawler output into a `BossData`, ready to deploy after you edit `_crawl()`.
</span>
